...
class MultiModalPolicy(nn.Module):
    """Joint + point cloud + tactile -> shared latent -> 4 action heads."""
    ...
    def __init__(
        ...
        *,
        ...
        tactile_pe: Optional[FourierFeatures] = None,
        ...
        super().__init__()
        if joint_dim <= 0:
            raise ValueError("joint_dim must be > 0")
        if point_dim < 3:
            raise ValueError("point_dim must be >= 3")
        if tactile_dim < 0:
            raise ValueError("tactile_dim must be >= 0")
        ...
        self.joint_enc = VectorEncoder(
            joint_dim, joint_feat_dim, pe=joint_pe, hidden_dims=(256, 256), dropout=trunk_dropout
        )
        ...
        self.use_tactile = tactile_dim > 0
        if self.use_tactile:
            self.tactile_enc = VectorEncoder(
                tactile_dim, tactile_feat_dim, pe=tactile_pe, hidden_dims=(128, 128), dropout=trunk_dropout
            )
        else:
            self.tactile_enc = None
            tactile_feat_dim = 0

        trunk_in = joint_feat_dim + point_feat_dim + tactile_feat_dim
    ...
    def forward(
        ...
        *,
        joint: Tensor,
        point_cloud: Tensor,
        tactile: Optional[Tensor] = None,
        point_mask: Optional[Tensor] = None,
    ) -> Dict[str, Tensor]:
        """Inputs:
          joint: (B, J)
          point_cloud: (B, N, C)
          tactile: (B, T) or None (if tactile_dim==0)
          point_mask: (B,N) bool mask, optional

        Returns:
          dict head_name -> (B, head_dim)
        """
        ...
        if self.use_tactile:
            if tactile is None:
                raise ValueError("tactile input is required because tactile_dim>0")
            tactile_feat = self.tactile_enc(tactile)
            fused = torch.cat([joint_feat, pc_feat, tactile_feat], dim=-1)
        else:
