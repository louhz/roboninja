...
class MPMSimulator:
    def __init__(self, dim, quality, gravity, horizon, max_steps_local, max_steps_global, ckpt_dest):
        ...
        self.ground_friction = 2.0
                ...
                # Call setup_tactile_sensor(...) BEFORE build()
        self.tactile_cfg = None
        self.has_tactile_sensor = False

        # These are set in init_tactile_sensor() during build()
        self.tactile_record_per_particle = False
        self.tactile_one_sided = False
        self.tactile_front_sign = 1          # +1: front side is +normal, -1: front side is -normal
    ...
    def build(self, agent, statics, x, mat, used, p_rho, body_id):
        ...
        self.statics = statics
                # tactile sensor (must be allocated after particles exist if record_per_particle=True)
        ...
        if x is not None:
            ...
            if self.has_particles and (self.tactile_cfg is not None):
    ...
    def setup_tactile_sensor(
        ...
        pulse_on_entry=True,
        one_sided=False,
        front_side="positive",          # "positive" or "negative" (only used if one_sided=True)
        record_per_particle=False,
    ):
        """
        Configure a rectangular plate tactile sensor.

        Geometry:
          - center: (dim,) world coords
          - length/height: extents along plate tangential axes (world units)
          - normal_axis (3D): which axis is the plate normal:
                normal_axis=0 -> plate is in (y,z), thickness on x
                normal_axis=1 -> plate is in (x,z), thickness on y
                normal_axis=2 -> plate is in (x,y), thickness on z
            Tangential axes are the other axes in ascending order:
                if normal_axis=1 -> tangential axes are [0,2] => length on x, height on z
          - thickness: slab thickness along normal axis (default: 2*dx)

        Outputs:
          - contact signal: any particle currently inside slab
          - pulse: by default, only when a particle ENTERS the slab this frame
        """
        center = np.array(center, dtype=DTYPE_NP)
        assert center.shape == (self.dim,), f"center must be shape ({self.dim},), got {center.shape}"

        if thickness is None:
            thickness = 2.0 * self.dx  # good default for MPM grids (avoid skipping)

        # Compute half-size AABB in world coords
        if self.dim == 3:
            assert normal_axis in (0, 1, 2), "normal_axis must be 0/1/2 for dim=3"
            tangential = [a for a in range(3) if a != normal_axis]  # e.g. if normal=1 -> [0,2]
            ...
            # In 2D we treat sensor as an axis-aligned rectangle:
            # length on x, height on y. normal is used only for pulse direction.
        ...
        # Which side is "front" for one-sided pulses
        front_sign = 1
        if isinstance(front_side, str):
            fs = front_side.lower()
            if fs.startswith("neg"):
                front_sign = -1
            else:
                front_sign = 1
        else:
            front_sign = int(front_side)
            front_sign = 1 if front_sign >= 0 else -1
    ...
    def init_tactile_sensor(self):
        """
        Called inside build() AFTER particles are set up.
        ...
        # optional per-particle outputs for the latest sensed frame
    ...
    def tactile_contains(self, x):
        """
        Axis-aligned slab/box test: returns 0/1 (int).
        """
    ...
    def tactile_sense_particles(self, f: ti.i32):
        """
        Compute tactile_contact / tactile_count for frame f (level signal),
        and tactile_pulse for frame f (pulse signal, default: only on entry).
        """
        ...
        for p in range(self.n_particles):
            if self.particles_ng[f, p].used:
                ...
                if inside_now:
                    ...
                    # pulse: default is only when particle ENTERS sensor slab
                    entering = inside_now
                    if ti.static(self.tactile_pulse_on_entry):
                        inside_prev = 0
                        if f > 0:
                            x_prev = self.particles[f - 1, p].x
                            inside_prev = self.tactile_contains(x_prev)
                        entering = inside_now * (1 - inside_prev)  # 1 if entered this frame
    ...
    def get_tactile(self, f=None):
        """
        Returns dict with contact/pulse/count for a given local frame (defaults to current).
        """
    ...
    def get_tactile_particles(self):
        """
        Per-particle contact/pulse for the LAST sensed frame (only if record_per_particle=True).
        """
    ...
    def substep(self, f, is_none_action):
        ...
        if self.has_particles:
            ...
            if self.has_tactile_sensor:
                # Sense on the newly-updated frame (f+1)
