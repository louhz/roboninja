"""
Multi-stage tactile + point cloud encoder-decoder Transformer
with tri-plane (tri-linear plane) state + stage-conditioned action chunking
+ NEW: object selection conditioning via one-hot -> linear embedding.

Key additions:
- Input `object_onehot: [B, num_objects]` (one-hot or soft distribution)
- `object_proj: Linear(num_objects -> d_model)` to embed object identity
- Combined context embedding: e_ctx = stage_emb + object_emb
  injected into encoder tokens + decoder tokens
- Tri-plane writer conditioning now includes object_emb as well:
  cond = [global_out, pooled_action, object_emb]
"""
...
class FourierPosEnc3D(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor) -> torch.Tensor:
        ...
        """
        x = xyz.unsqueeze(-1) * self.freqs.view(1, 1, 1, -1) * torch.pi  # [B,N,3,B]
        sin = torch.sin(x)
        cos = torch.cos(x)
...
class PointTactileTokenizer(nn.Module):
    ...
    NOTE:
    `ctx_emb` is a d_model vector injected into all tokens (stage+object combined).
    """
    ...
    def forward(
        ...
        ctx_emb: torch.Tensor                      # [B,d_model] (stage+object)
...
class ActionChunkDecoder(nn.Module):
    ...
    def select_chunk_size(self, memory: torch.Tensor) -> int:
        ...
        pooled = memory[:, 0]  # typically stage token
...
class ModelConfig:
    ...
    tokenizer: TokenizerConfig = field(default_factory=lambda: TokenizerConfig(
        point_feat_dim=6, tactile_feat_dim=8, d_model=256, include_tactile_tokens=True
    ))
    ...
    # NEW: object conditioning
    num_objects: int = 1

    # If None: auto = 3 * d_model (global_out + pooled_action + object_emb)
...
class MultiStageTactilePointCloudTransformer(nn.Module):
    ...
    One macro-step forward:
      ...
      - build tokens with tactile registration + (stage+object) context embedding
      - encode tokens
      - choose chunk size (fixed by stage or adaptive)
      - decode action chunk (ctx injected)
      - update tri-plane (cond includes object embedding)
    """
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        # Explicit stage token (now “ctx token”: stage+object)
        self.stage_token_proj = nn.Linear(d, d)
    ...
    def forward_macro_step(
        ...
        if object_onehot is None:
            object_onehot = torch.zeros((B, self.cfg.num_objects), device=device, dtype=point_xyz.dtype)
        else:
            ...
            object_onehot = object_onehot.to(device=device, dtype=point_xyz.dtype)

        e_obj = self.object_proj(object_onehot)  # [B,d]

        # combined context embedding
        e_ctx = e_stage + e_obj  # [B,d]
        ...
        tokens_wo_stage, aux_tok = self.tokenizer(
        ...
        ctx_tok = self.stage_token_proj(e_ctx).unsqueeze(1)  # [B,1,d]
        tokens = torch.cat([ctx_tok, tokens_wo_stage], dim=1)  # [B, 1+S0, d]
        ...
        # token order: [ctx_tok, global_tok, ...]
        global_out = memory[:, 1, :]  # [B,d]
        pooled_a = self.action_pool(pred_actions).mean(dim=1)  # [B,d]

        cond = torch.cat([global_out, pooled_a, e_obj], dim=-1)  # [B,3d] (includes object)
...
if __name__ == "__main__":
    ...
    cfg = ModelConfig(
        plane=TriPlaneConfig(res=64, channels=32),
        tokenizer=TokenizerConfig(
            point_feat_dim=6, tactile_feat_dim=8, d_model=256,
            include_tactile_tokens=True, k_tactile=8, tactile_temp=0.05
        ),
        encoder=EncoderConfig(d_model=256, nhead=8, num_layers=4, dim_feedforward=1024, dropout=0.1),
        decoder=DecoderConfig(
            d_model=256, nhead=8, num_layers=4, dim_feedforward=1024, dropout=0.1,
            action_dim=7, max_chunk_size=16,
            stage_chunk_sizes=(2, 8),
            use_adaptive_chunking=False,
            candidate_chunk_sizes=(1, 2, 4, 8, 16),
        ),
        num_stages=2,
        num_objects=5,          # <-- NEW: number of object categories
        writer_cond_dim=None,   # auto = 3*d_model
    )

    model = MultiStageTactilePointCloudTransformer(cfg)

    # Dummy batch
    B, N, M = 2, 512, 32
    dp, dt, da = cfg.tokenizer.point_feat_dim, cfg.tokenizer.tactile_feat_dim, cfg.decoder.action_dim

    device = torch.device("cpu")
    model.to(device)

    point_xyz = torch.randn(B, N, 3, device=device)
    point_feats = torch.randn(B, N, dp, device=device)

    tactile_xyz = torch.randn(B, M, 3, device=device)
    tactile_feats = torch.randn(B, M, dt, device=device)

    stage = torch.zeros(B, device=device, dtype=torch.long)  # stage 0

    # NEW: object one-hot
    object_id = torch.tensor([0, 3], device=device)  # [B]
    object_onehot = F.one_hot(object_id, num_classes=cfg.num_objects).float()  # [B,num_objects]

    aabb_min = torch.tensor([-2.0, -2.0, -2.0], device=device)
    aabb_max = torch.tensor([ 2.0,  2.0,  2.0], device=device)

    planes = model.init_planes(B, device=device)

    # Teacher forcing GT actions
    Kmax = cfg.decoder.max_chunk_size
    actions_gt = torch.randn(B, Kmax, da, device=device)
    ...
    out = model.forward_macro_step(
        ...
        tactile_feats=tactile_feats,
        stage=stage,
        object_onehot=object_onehot,  # <-- NEW
        aabb_min=aabb_min,
        aabb_max=aabb_max,
        planes=planes,
    ...
    K = int(out["K"].item())
    print("pred_actions:", pred_actions.shape, "K:", K)

    # Example losses (dummy render/label)
    loss_act = action_mse_loss(pred_actions, actions_gt[:, :K, :])
    gt_render = torch.zeros_like(out["render_feat"])
    loss_render = render_l1_loss(out["render_feat"], gt_render)
    success_label = torch.zeros(B, device=device)
    loss_succ = success_bce_loss(out["success_prob"], success_label)

    loss = 1.0 * loss_act + 0.1 * loss_render + 0.1 * loss_succ
